...
import { Building2, Globe, Phone, Mail, Calendar, Clock, Plus } from 'lucide-react';
...
export default function Index({ companyInfo }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    ...
    founded_year: companyInfo?.founded_year?.toString() || '',
    business_hours: companyInfo?.business_hours || '',
    logo: null as File | null,
    additional_data: companyInfo?.additional_data ? JSON.stringify(companyInfo.additional_data, null, 2) : '',
  });
  ...
  const [logoPreview, setLogoPreview] = React.useState<string | null>(
    companyInfo?.logo_url ? `/storage/${companyInfo.logo_url}` : null
  );
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setLogoPreview(companyInfo?.logo_url ? `/storage/${companyInfo.logo_url}` : null);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const route_name = companyInfo ? 'company-info.update' : 'company-info.store';
    const route_params = companyInfo ? [companyInfo.id] : [];
    
    post(route(route_name, ...route_params), {
      onSuccess: () => {
        Swal.fire({
          title: 'Success!',
          text: `Company information ${companyInfo ? 'updated' : 'created'} successfully`,
          icon: 'success',
          timer: 4000,
  ...
  // If no company info exists, show create form
  if (!companyInfo) {
    return (
      <DashboardLayout title="Company Information">
        ...
        <Card>
          ...
          <CardContent>
            <Link href={route('company-info.create')}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end">
            <Button 
              ...
              disabled={processing}
            >
              {companyInfo ? 'Update' : 'Save'} Company Information
            </Button>
          </div>
        </form>
      </div>
